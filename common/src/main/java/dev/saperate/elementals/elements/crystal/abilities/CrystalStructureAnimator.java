...
/**
 * CrystalStructureAnimator
 * <p>
 * Igual a {@code MudStructureAnimator} (pacote {@code mud.abilities}), só
 * que com sons de ametista em vez de lama. Anima uma estrutura de blocos
 * "crescendo" camada por camada (de baixo pra cima), ficando de pé por um
 * tempo e depois "recolhendo"/sumindo do mesmo jeito, restaurando o
 * {@link BlockState} original de cada posição.
 * <p>
 * Não compartilha código com {@code MudStructureAnimator} diretamente
 * porque as habilidades de Mud dependem dela ser package-private dentro de
 * {@code mud.abilities} (mesma decisão de design já documentada em
 * {@code AbilitySupport}: cada subbending tem seu próprio pacote
 * autocontido, sem depender de classes internas de outro).
 */
public class CrystalStructureAnimator {
    ...
    public CrystalStructureAnimator(ServerLevel world, List<List<BlockPos>> layers,
    ...
    private void placeLayer(List<BlockPos> layer) {
        ...
        if (!layer.isEmpty()) {
            ...
            world.playSound(null, ref, SoundEvents.AMETHYST_BLOCK_PLACE, SoundSource.BLOCKS,
    ...
    private void restoreLayer(List<BlockPos> layer) {
        ...
        if (!layer.isEmpty()) {
            BlockPos ref = layer.get(layer.size() / 2);
            world.playSound(null, ref, SoundEvents.AMETHYST_BLOCK_BREAK, SoundSource.BLOCKS,
